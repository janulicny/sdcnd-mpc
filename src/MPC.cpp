 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t t = 0; t < N; t++) {
      // penalty for being far from reference trajectory
      fg[0] += 30*CppAD::pow(vars[cte_start + t], 2);
      // penalty for having different orientation than on reference trajectory
      fg[0] += 10*CppAD::pow(vars[epsi_start + t], 2);    
      // penalty for low speed
      fg[0] += 20*CppAD::exp(-vars[v_start + t]/50);
    }
    
    for (size_t t = 0; t < N - 1; t++) {
      // penalty for using steering
      fg[0] += 1*CppAD::pow(vars[delta_start + t], 2);
      // penalty for using longitudinal control
      fg[0] += 0.1*CppAD::pow(vars[a_start + t], 2);
    }
    
    for (size_t t = 1; t < N - 1; t++) {
      // penalty for changing angle of steering
      fg[0] += 1000000*CppAD::pow(vars[delta_start + t] - vars[delta_start + t - 1], 2);
      // penalty for changing longitudinal control
    ...
    for (size_t t = 1; t < N; t++) {
      ...
      // changed this according to tip from Slack channel
      fg[1 + cte_start + t] =
